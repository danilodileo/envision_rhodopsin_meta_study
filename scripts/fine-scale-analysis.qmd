...
# ------------------------------
# column_split order: Offshore×Winter, Offshore×Spring, Offshore×Summer,
#                     Coast×Winter,   Coast×Spring,   Coast×Summer  (6 groups)
# column_gap = c(0,0,6,0,0): no gap within same site, 6 mm between sites.
# Site fill: same colour per site → 3 adjacent zero-gap blocks merge visually.

ha_col <- HeatmapAnnotation(
  Site = anno_block(
    gp = gpar(
      fill = c("#DDEEFF", "#DDEEFF", "#DDEEFF", "#FFE8CC", "#FFE8CC", "#FFE8CC"),
      col  = NA
    ),
    labels     = c("Offshore", "", "", "Coast", "", ""),
    labels_gp  = gpar(fontsize = 9, fontface = "bold")
  ),
  Season = anno_block(
    gp        = gpar(fill = NA, col = "grey50"),
    labels    = c("Winter", "Spring", "Summer", "Winter", "Spring", "Summer"),
    labels_gp = gpar(fontsize = 8)
  ),
  Day = anno_text(
    as.character(row_annot$day),
    gp       = gpar(fontsize = 8),
    rot      = 0,
    just     = "center",
    location = 0.5
...
ht1 <- Heatmap(
  ...
  column_gap = unit(c(0, 0, 6, 0, 0), "mm"),
  col = colorRamp2(
...
ht2 <- Heatmap(
  ...
  show_column_names = FALSE,
  border = TRUE,
  col = colorRamp2(c(0, max_val_perc/2, max_val_perc), c("white", "#A1D99B", "#238B45")),
  row_names_gp = gpar(fontsize = 10), 
...
ht3 <- Heatmap(
  ...
  col = colorRamp2(c(0, 150, 300), c("white", "#FDAE6B", "#D94801")),
...
# Genus | Family annotation aligned with heatmap row order
genome_genus_map <- top30genomes.prd.abundance %>%
  distinct(genome_with_family = as.character(genome_with_family), genus, family)
genus_vector  <- genome_genus_map$genus[match(row_order_ref,  genome_genus_map$genome_with_family)]
family_vector <- genome_genus_map$family[match(row_order_ref, genome_genus_map$genome_with_family)]
genus_family_labels <- paste0(genus_vector, " | ", family_vector)

ra_genus <- rowAnnotation(
  "Genus | Family" = anno_text(genus_family_labels, gp = gpar(fontsize = 10)),
  show_annotation_name = TRUE,
...
ht_list <- ht1 + ht3 + ht2 + ht4 + ra_genus

png("figures/Figure4.png", width = 20, height = 12, units = "in", res = 300)
draw(
  ht_list,
...
cairo_pdf("figures/Figure4.pdf", width = 20, height = 12)
draw(
  ht_list,
  heatmap_legend_side    = "bottom",
  annotation_legend_side = "bottom",
...
fig3b <- ggplot(
  ...
  scale_color_manual(name = "Season", values = c("green", "red", "blue")) +
...
# Facet order: group panels by family (following the manuscript's existing
# family_class_colors order), with archaeal genomes (domain == "Archaea",
# class Poseidoniia in this dataset) placed together at the very end;
# alphabetical by genome accession within each family.
genome_facet_order <- counts.rhodo.genomes %>%
  distinct(genome, family, domain) %>%
  filter(genome %in% unique(plot_df$genome)) %>%
  mutate(
    is_archaea  = domain == "Archaea",
    family_rank = match(family, str_remove(names(family_class_colors), "_[^_]+$")),
    family_rank = ifelse(is.na(family_rank), Inf, family_rank)
  ) %>%
  arrange(is_archaea, family_rank, family, genome) %>%
  pull(genome)

plot_df <- plot_df %>%
  mutate(genome = factor(genome, levels = genome_facet_order))
...
# --- Color-code the column labels by family (same palette as family_class_colors) ---
# Column labels are "<genome>_<family>_<genus>[†]"; genus spelling is
# inconsistent between individual.abundance and counts.rhodo.genomes
# (e.g. "MGIIb.O2" vs "MGIIb-O2" for the same genome), so match on the
# genome accession only (the leading token), which is unambiguous.
prd.genome_class <- counts.rhodo.genomes %>%
  distinct(genome, family, class) %>%
  group_by(genome) %>%
  slice(1) %>%
  ungroup() %>%
  mutate(
    family_class = paste0(family, "_", class),
    family_class = ifelse(family_class %in% names(family_class_colors),
                           family_class, "other_family_other_class")
  )

prd.col_stripped     <- sub("†$", "", colnames(prd.cor_mat))
prd.genome_ids_sorted <- prd.genome_class$genome[order(-nchar(prd.genome_class$genome))]
prd.find_genome <- function(x) {
  hit <- prd.genome_ids_sorted[startsWith(x, paste0(prd.genome_ids_sorted, "_"))]
  if (length(hit) == 0) NA_character_ else hit[1]
}
prd.col_genome_id <- vapply(prd.col_stripped, prd.find_genome, character(1))

prd.col_label_colors <- setNames(
  family_class_colors[prd.genome_class$family_class[match(prd.col_genome_id, prd.genome_class$genome)]],
  colnames(prd.cor_mat)
)

# Inject the per-label colors into the already-built Heatmap object: passing
# column_names_gp directly to pheatmap() errors ("matched by multiple actual
# arguments"), since the wrapper already builds it internally from
# fontsize_col/fontface_col. Post-hoc slot assignment keeps pheatmap()'s
# sizing/layout untouched and only overrides label colors.
stopifnot(identical(fig3a@column_names_param$labels, colnames(prd.cor_mat)))
fig3a@column_names_param$gp$col <- prd.col_label_colors[fig3a@column_names_param$labels]
```
...
fig3_final <- as.ggplot(fig3a) + fig3b +
  plot_annotation(tag_levels = 'A')

ggsave("figures/Figure3.pdf", plot = fig3_final, width = 20, height = 11)
ggsave("figures/Figure3.png", plot = fig3_final, width = 20, height = 11, dpi = 300)
```

```{r spearman-by-season-and-site}
#| echo: false
#| warning: false

# Add site/season metadata to the correlation data frame
prd.genome.correlation.meta <- prd.genome.correlation %>%
  left_join(
    counts.rhodo.genomes %>% distinct(sample, genome, site, season),
    join_by(sample, genome)
  )

# Helper: run Spearman cor.test and extract rho, raw p, BH-adjusted p
run_spearman <- function(df, group_var) {
  df %>%
...
season_corr <- run_spearman(prd.genome.correlation.meta, "season") %>%
  rename(group = season) %>%
  mutate(grouping = "Season")

site_corr <- run_spearman(prd.genome.correlation.meta, "site") %>%
  rename(group = site) %>%
  mutate(grouping = "Site")

group_corr_stats <- bind_rows(season_corr, site_corr) %>%
  select(grouping, group, n, rho, p_raw, p_adj)
...
kable(
  group_corr_stats %>%
    mutate(
      rho   = round(rho, 3),
      p_raw = format.pval(p_raw, digits = 3, eps = 1e-16),
      p_adj = format.pval(p_adj, digits = 3, eps = 1e-16)
    ),
  col.names = c("Grouping", "Group", "N", "Spearman ρ", "p (raw)", "p (BH-adjusted)"),
  caption = "Group-wise Spearman correlations between log(Total TPM) and log(Rhodopsin contribution)"
)
```
...
group_by(sample, kegg_ko, genome, site, day) %>%
  ...
  summarise(avg = mean(sum_perc),
            sd = sd(sum_perc), .groups = "drop") %>%
  # day is numeric in the source table; scale_y_discrete() below needs a
  # discrete y, otherwise it silently drops the axis tick labels entirely.
...
#| fig-width: 14
#| fig-height: 7
porti.bubble.viz <- (porti.bubble.1 + porti.bubble.2) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")

porti.bubble.viz

# cairo_pdf: some KO descriptions contain Greek letters / en-dashes that the
# base pdf() device's font can't encode (see supplementary-figure-7 fix).
ggsave("figures/porti-bubble-visualization.pdf", plot = porti.bubble.viz, width = 14, height = 7, device = cairo_pdf)
ggsave("figures/porti-bubble-visualization.png", plot = porti.bubble.viz, width = 14, height = 7, dpi = 300)
```
...
#| fig-width: 14
#| fig-height: 7
pela.bubble.viz <- (pela.bubble.1 + pela.bubble.2) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")

pela.bubble.viz

# cairo_pdf: some KO descriptions contain em-dashes that the base pdf()
# device's font can't encode (see supplementary-figure-7 fix).
ggsave("figures/pela-bubble-visualization.pdf", plot = pela.bubble.viz, width = 14, height = 7, device = cairo_pdf)
ggsave("figures/pela-bubble-visualization.png", plot = pela.bubble.viz, width = 14, height = 7, dpi = 300)
...
#| fig-width: 10
#| fig-height: 14
# Poseidoniaceae only, Spring + Summer (Coast). DOC is a sample-level
# environmental variable (identical across every genome on a given day), so
# it's shown once, in its own real units (not scaled), rather than repeated
# per genome with a shared z-scored axis that flattened its actual trend.
# chla_tot is dropped: not discussed for Poseidoniaceae in the main text.
posei.env.df <- individual.abundance %>%
  filter(
    site == 'Coast',
  ...
  distinct(day, sample, season) %>%
  left_join(metadata %>% select(sample, doc), by = "sample") %>%
  mutate(season = factor(season, levels = c("Spring", "Summer")))

posei.env <- ggplot(posei.env.df, aes(x = as.numeric(day), y = doc)) +
  geom_line(color = "#0072B2", linewidth = 1) +
  geom_point(color = "#0072B2", size = 2.5) +
  facet_wrap(~season, nrow = 1) +
  theme_bw(base_size = 16) +
  labs(x = NULL, y = "DOC (µM C)", title = "Environmental context (Coast)") +
  theme(
...
# Rhodopsin is genome-specific, in its own unit (% of genome transcription) —
# kept raw as well, one panel per genome x season.
posei.rhodo.df <- individual.abundance %>%
  filter(
    site == 'Coast',
    season %in% c("Spring", "Summer"),
    genome %in% (top30genomes.prd.abundance %>% distinct(genome) %>% pull(genome)),
  ...
  group_by(day, sample, site, season, genome) %>%
  summarise(rhodopsin = sum(perc), .groups = 'drop') %>%
  mutate(season = factor(season, levels = c("Spring", "Summer")))

posei.rhodo <- ggplot(posei.rhodo.df, aes(x = as.numeric(day), y = rhodopsin)) +
  geom_line(color = "black", linewidth = 1) +
  geom_point(color = "black", size = 2.5) +
  facet_grid(genome ~ season, scales = "free_y") +
  theme_bw(base_size = 16) +
  labs(x = "Day", y = "Rhodopsin (% of genome transcription)",
       title = "Poseidoniaceae genomes") +
  theme(
    strip.text.x = element_text(size = 15, face = "bold"),
    # smaller than strip.text.x: rotated accession strings (~16 chars) at
    # 15pt clipped against the row height even at fig-height 12in
...
posei.line <- posei.env / posei.rhodo +
...
ggsave("figures/supplementary-figure-6.pdf", plot = posei.line, width = 10, height = 14)
ggsave("figures/supplementary-figure-6.png", plot = posei.line, width = 10, height = 14, dpi = 300)
```

```{r supplementary-figure-7}
#| fig-width: 12
#| fig-height: 6
# Same single-panel-per-family layout/legend as the original suppl-fig4
# (z-scored "Scaled value" axis, one overlaid panel per family), but:
# (1) Puniceispirillaceae keeps only metabat2.225 — GCA_943789275.1 was
#     Poseidoniaceae, wrongly included before (see supplementary-figure-6);
# (2) color/linetype are swapped from the original: color now encodes
#     variable (rhodopsin/doc/chla_tot) so they're never the same color,
#     and linetype encodes genome — the original (color = genome,
#     linetype = variable) made chla_tot/doc indistinguishable in color.
var_colors <- c(rhodopsin = "#000000", doc = "#0072B2", chla_tot = "#D55E00")
var_labs   <- c(rhodopsin = "Rhodopsin", doc = "DOC", chla_tot = "Chl a")

punice.line.df <- individual.abundance %>%
  filter(
    site == 'Coast',
    season == "Spring",
    genome == 'metabat2.225',
  ...
  summarise(rhodopsin = sum(perc), .groups = 'drop') %>%
  left_join(metadata %>% select(sample, doc, chla_tot), by = "sample") %>%
  mutate(across(c(rhodopsin, doc, chla_tot), ~ scale(.)[,1])) %>%
  pivot_longer(cols = c(rhodopsin, doc, chla_tot), names_to = "variable")

punice.line <- ggplot(punice.line.df, aes(
    x = as.numeric(day), y = value,
    color = variable, linetype = genome_family
  )) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = var_colors, labels = var_labs) +
  scale_linetype_manual(values = c("metabat2.225_Puniceispirillaceae" = "solid")) +
  theme_bw() +
  facet_wrap(~season+site) +
  labs(x = "Day", y = "Scaled value", color = "Variable", linetype = "Genome",
...
suppl.fig7 <- punice.line + porti.line +
  plot_layout(guides = 'collect') +
  plot_annotation(tag_levels = 'A') &
  theme(plot.tag = element_text(size = 20, face = "bold"))
...
ggsave("figures/supplementary-figure-7.pdf", plot = suppl.fig7, width = 12, height = 6)
ggsave("figures/supplementary-figure-7.png", plot = suppl.fig7, width = 12, height = 6, dpi = 300)
```
