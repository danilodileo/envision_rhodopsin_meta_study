...
ko_list <- c("K00001", "K00002", "K21395")  # your list

ko_to_pathway <- lapply(ko_list, function(ko) {
  res <- tryCatch(keggGet(ko)[[1]]$PATHWAY, error = function(e) NULL)
  if (!is.null(res)) data.frame(KO = ko, pathway_id = names(res), pathway_name = unname(res))
})

pathway_table <- do.call(rbind, ko_to_pathway)
```
