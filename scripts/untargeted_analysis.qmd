...
# Family and class palette (same as fine-scale-analysis.qmd / Overall_community...
# .qmd, kept identical so the same family always gets the same color across
# manuscript figures)
...
# ── 2. Build summary with taxonomy ───────────────────────────────────────────
class_summary <- plot_data %>%
  left_join(
    counts.rhodo.genomes %>%
      filter(genome %in% top30.rhodo.genomes) %>%
      distinct(genome, domain, family, genus) %>%
      rename(tax_family = family, tax_genus = genus),
    join_by(genome)
  ) %>%
  mutate(genome_family = paste0(genome, '_', tax_family, '_', tax_genus)) %>%
  # left_join(dominance, join_by(genome_family)) %>%
  # mutate(genome_family = case_when(
  #   dominance == "coast" ~ paste0(genome_family, '*'),
  #   TRUE ~ genome_family
  # )) %>%
  filter(!is.na(class)) %>%
  group_by(genome_family, class, domain, tax_family, tax_genus) %>%
  summarise(
...
# Taxonomic order
col_order <- class_summary %>%
  distinct(genome_family, domain, tax_family, tax_genus) %>%
  arrange(domain, tax_family, tax_genus) %>%
  pull(genome_family)

# ── 4. Build matrices (columns in taxonomic order) ───────────────────────────
cor_matrix <- class_summary %>%
  select(genome_family, class, mean_cor) %>%
  pivot_wider(names_from = genome_family, values_from = mean_cor, values_fill = 0) %>%
  column_to_rownames("class") %>%
  as.matrix() %>%
  .[, col_order]                                # apply taxonomic order
...
sig_matrix <- class_summary %>%
  ...
  column_to_rownames("class") %>%
  as.matrix() %>%
  .[, col_order]                                # same order

# ── 5. Plot ───────────────────────────────────────────────────────────────────
pal    <- colorRampPalette(c("#2166AC", "#FFFFFF", "#D6604D"))(100)
breaks <- seq(-0.7, 0.7, length.out = 101)  # actual data range is -0.55/0.80; -1/1 left most of the palette unused
...
f5a <- pheatmap(
  ...
  cellheight       = 15,
  cluster_rows     = TRUE,
  cluster_cols     = FALSE,   # taxonomic order, no clustering on cols
  treeheight_row   = 5,
  display_numbers  = sig_matrix,
  number_color     = "black",
  name             = "rho"
...
# --- Color-code the column labels by family (same family_class_colors
# palette/pattern as Figure 3A in fine-scale-analysis.qmd) ---
# Match on the genome accession only (leading token of genome_family), not
# the full string: genus spelling can differ across tables, so a full-string
# match can silently miss genomes (see fine-scale-analysis.qmd fig3a fix).
f5a.genome_class <- counts.rhodo.genomes %>%
...
f5a.col_labels        <- colnames(cor_matrix)
f5a.genome_ids_sorted  <- f5a.genome_class$genome[order(-nchar(f5a.genome_class$genome))]
f5a.find_genome <- function(x) {
  hit <- f5a.genome_ids_sorted[startsWith(x, paste0(f5a.genome_ids_sorted, "_"))]
  if (length(hit) == 0) NA_character_ else hit[1]
}
f5a.col_genome_id <- vapply(f5a.col_labels, f5a.find_genome, character(1))

f5a.col_colors <- setNames(
  family_class_colors[f5a.genome_class$family_class[match(f5a.col_genome_id, f5a.genome_class$genome)]],
  f5a.col_labels
)

# Post-hoc slot assignment: pheatmap()'s own column_names_gp already consumes
# fontsize_col/fontface_col internally, so passing column_names_gp a second
# time via `...` errors ("matched by multiple actual arguments").
stopifnot(identical(f5a@column_names_param$labels, colnames(cor_matrix)))
f5a@column_names_param$gp$col <- f5a.col_colors[f5a@column_names_param$labels]
```
...
## Looking into three genomes

```{r correlation-plot-pelagibacter-transporters}
selected.genomes <- transporters.individual %>%
  filter(genome %in% c("GCA_003279715.1", "GCF_943788075.1", "GCA_028097645.1"),
         ko %in% c('K17315', 'K21395', 'K09969', 'K04641'))


ko_targets <- c('K17315', 'K21395', 'K09969')  # everything except K04641

# Prepare wide format once
wide_data <- selected.genomes %>%
  select(genome, perc, ko, sample) %>%
  group_by(genome, sample, ko) %>%
  summarise(sum_perc = sum(perc), .groups = 'drop') %>%
  pivot_wider(names_from = ko, values_from = sum_perc, values_fill = 0) %>%
  rename(
    "K09969\nL-amino acid"            = K09969,
    "K21395\ngtrC; TRAP-type"         = K21395,
    "K17315\nglcE; glucose/mannose" = K17315
  )

highlight_pairs <- tibble(
  genome = c("GCA_003279715.1", "GCF_943788075.1", "GCA_028097645.1"),
  ko     = c("K17315\nglcE; glucose/mannose",
             "K21395\ngtrC; TRAP-type",
             "K09969\nL-amino acid")
)
...
# Build plot_data
plot_data <- wide_data %>%
  pivot_longer(cols = c("K09969\nL-amino acid",
                        "K21395\ngtrC; TRAP-type",
                        "K17315\nglcE; glucose/mannose"),
               names_to = "ko", values_to = "value") %>%
  left_join(highlight_pairs %>% mutate(correlated = TRUE),
            by = c("genome", "ko")) %>%
  mutate(correlated = replace_na(correlated, FALSE))

# Conta ORF per genome x KO
orf_counts <- selected.genomes %>%
  distinct(orf, genome, ko) %>%
  group_by(genome, ko) %>%
  summarise(n_orf = n(), .groups = 'drop') %>%
  mutate(ko = recode(ko,
                     "K09969"  = "K09969\nL-amino acid",
                     "K21395"  = "K21395\ngtrC; TRAP-type",
                     "K17315"  = "K17315\nglcE; glucose/mannose"
...
# Aggiungi i conteggi a plot_data
tplot_data <- plot_data %>%
  left_join(orf_counts, by = c("genome", "ko"))

f5b <- ggplot(tplot_data, aes(x = K04641, y = value)) +
  geom_smooth(data = filter(tplot_data, correlated),
              method = 'lm', formula = y ~ x, color = "steelblue") +
  geom_smooth(data = filter(tplot_data, !correlated),
              method = 'lm', formula = y ~ x, color = "grey70", linetype = "dashed") +
  geom_point(aes(color = correlated), size = 1.5) +
  geom_text(data = tplot_data %>% distinct(genome, ko, n_orf),
            aes(label = paste0("n ORFs = ", n_orf)),
            x = Inf, y = Inf, hjust = 1.1, vjust = 1.5,
            size = 3, color = "black", inherit.aes = FALSE) +
  scale_color_manual(values = c("TRUE" = "steelblue", "FALSE" = "grey70"),
                     labels = c("TRUE" = "correlated", "FALSE" = "not correlated"),
                     name = "") +
  facet_grid(ko ~ genome) +
  labs(x = "K04641 (rhodopsin)(%)", y = "Transporter(%)") +
  theme_bw() +
...
```{r fig-5}
#| fig-height: 10
f5a_gg <- as.ggplot(f5a)
f5b_gg <- as.ggplot(f5b)

x <- wrap_plots(f5a_gg, f5b_gg, ncol = 1) + 
  plot_layout(heights = c(1, 1)) + plot_annotation(tag_levels = 'A') # adjust ratio if one heatmap is taller

ggsave('fig5.jpeg', x, width = 10, height = 16, units = "in", dpi = 300)

# cairo_pdf: guards against the base pdf() device mangling any non-ASCII
# character in genome/KO labels (see supplementary-figure-7 fix in
# fine-scale-analysis.qmd for the same issue).
ggsave("figures/fig5.pdf", plot = x, width = 10, height = 16, units = "in", device = cairo_pdf)
ggsave("figures/fig5.png", plot = x, width = 10, height = 16, units = "in", dpi = 300)
```
...
# group_split(family) sorts alphabetically, giving the fixed index below —
# used by the per-family chunks that follow (order chosen for narrative flow,
# not index order):
#   [[1]] CABXJW01            [[4]] Porticoccaceae      [[7]] Puniceispirillaceae
#   [[2]] D2472               [[5]] Poseidoniaceae      [[8]] Rhodobacteraceae
#   [[3]] Pelagibacteraceae   [[6]] Pseudothioglobaceae [[9]] Thalassarchaeaceae
plots_by_family <-
  plot_df %>%
  group_split(family) %>%
  map(~ ggplot(.x,
